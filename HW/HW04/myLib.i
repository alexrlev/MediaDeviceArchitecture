...
void setPixel4(int row, int col, unsigned char colorIndex) {
    ...
    videoBuffer[((row)*(240)+(col))/2] = pixel;

}

void drawHorizontalLine3(int row, int col, int width, unsigned short color) {
    for (int i = 0; i < width; i++) {
        setPixel3(row, col++, color);
    }
}

void drawHorizontalLine4(int row, int col, int width, unsigned char colorIndex) {
    for (int i = 0; i < width; i++) {
        setPixel4(row, col++, colorIndex);
    }
}

void drawVerticalLine3(int row, int col, int height, unsigned short color) {
    for (int i = 0; i < height; i++) {
        setPixel3(row++, col, color);
    }
}

void drawVerticalLine4(int row, int col, int height, unsigned char colorIndex) {
    for (int i = 0; i < height; i++) {
        setPixel4(row++, col, colorIndex);
    }
}
    ...
    for (int r = 0; r < height; r++) {
        if (col & 1) {
            if (width & 1) {
                ...
                if (width > 1) {
                    DMANow(3, &c, &videoBuffer[((r+row)*(240)+(col+1))/2], (2 << 23) | width/2);
                }
            } else {
                setPixel4(row + r, col, colorIndex);
                if (width > 2) {
                    DMANow(3, &c, &videoBuffer[((r+row)*(240)+(col+1))/2], (2 << 23) | (width/2-1));
                }
            ...
            if (width & 1) {
                if (width > 1) {
                    DMANow(3, &c, &videoBuffer[((r+row)*(240)+(col))/2], (2 << 23) | width/2);
                }
                setPixel4(row + r, col+width-1, colorIndex);
            } else {
                DMANow(3, &c, &videoBuffer[((r+row)*(240)+(col))/2], (2 << 23) | width/2);
