...
void update() {
	...
	if ((bCol > 0) && BUTTON_HELD(BUTTON_LEFT))
		bCol--;
	if ((bCol + bWidth < 240) && BUTTON_HELD(BUTTON_RIGHT))
		bCol++;
	if ((bRow > 0) && BUTTON_HELD(BUTTON_UP))
		bRow--;
	if ((bRow + bHeight < 160) && BUTTON_HELD(BUTTON_DOWN))
		bRow++;
